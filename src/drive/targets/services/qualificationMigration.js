...
export const migrateQualifications = async () => {
  ...
  const filesSettings = await client.query(client.find(DOCTYPE_FILES_SETTINGS).limitBy(1))
  const settings = filesSettings && filesSettings.data.length > 0 ? filesSettings.data[0] : null
  ...
  const filesByDate = await queryFilesFromDate(client, lastProcessedFileDate, BATCH_FILES_LIMIT)
  if (filesByDate.length < 1) {
    log('warn', 'No new file to process')
    return
  }
  lastProcessedFileDate = filesByDate[filesByDate.length - 1].cozyMetadata.updatedAt
  ...
  if (filesToMigrate.length < 1) {
    ...
    log('info', `Migrated ${migratedFiles.length} files with old qualifications`)

    // Get last updated file date 
    const findLastUpdatedFile = migratedFiles.sort((fileA, fileB) => {
      const dateA = get(fileA, 'data.attributes.cozyMetadata.updatedAt')
      const dateB = get(fileB, 'data.attributes.cozyMetadata.updatedAt')
      return new Date(dateB) - new Date(dateA) 
    })
    lastProcessedFileDate = findLastUpdatedFile[0].data.attributes.cozyMetadata.updatedAt
  }
