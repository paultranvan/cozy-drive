...
const addToClusters = async (clusters, albumsToSave) => {
  ...
  for (const photos of clusters) {
    //TODO: be sure the albumsToSave are correctly sorted
...
const recreateClusters = async (clusters, albumsToSave) => {
  for (const photos of clusters) {
    ...
    const album = createAutoAlbum(photos)
...
const hasDuplicate = (photos, photo) => {
  ...
  for (const newPhoto of newPhotos) {
    ...
    if (matchingAlbums.length > 0) {
      ...
      if (key in clusterize) {
        if (!hasDuplicate(clusterize[key], newPhoto)) {
        ...
        if (!hasDuplicate(photosToClusterize, newPhoto)) {
